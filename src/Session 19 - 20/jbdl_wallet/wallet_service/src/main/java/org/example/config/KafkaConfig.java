...
public class KafkaConfig {
    ...
    @KafkaListener(topics = TopicConstants.INITIATE_TRANSACTION_TOPIC, groupId = "successful_transactions")
    public void pollMessagesForBalanceUpdation(ConsumerRecord receivedMessage) throws JsonProcessingException {
        InitiateTransactionDto receivedData =  objectMapper.readValue(receivedMessage.value().toString(), InitiateTransactionDto.class);
        log.info("ReceivedData: {}", receivedData.toString());
        walletOperations.updateBalanceForUsers(receivedData);
    }
}
